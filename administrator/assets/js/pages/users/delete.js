    ...
    $('[data-ajax-delete-user]').on('click', function()
    {
        ...
        swal({
            ...
            preConfirm: function ()
            {
                ...
                {
                    $.post('index.php?c=users&m=delete_user', { id: self.data('ajax-delete-user') }, function(data, status, jqXHR)
                    {
                        if ( data.status == 'OK' )
                        {
                            xhr_status = 'OK';
    ...
    $('[data-ajax-delete-permission]').on('click', function()
    {
        ...
        swal({
            ...
            preConfirm: function ()
            {
                return new Promise(function (resolve)
                {
                    $.post('index.php?c=users&m=delete_permission', { id: self.data('ajax-delete-permission') }, function(data, status, jqXHR)
                    {
                        if ( data.status == 'OK' )
                        {
                            xhr_status = 'OK';
